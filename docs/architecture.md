...
**API keys** protect the SDK ingestion endpoints (`POST /api/logs`, `GET /api/logs`, `DELETE /api/logs`). Keys use a `lw_` prefix, are stored bcrypt-hashed in MongoDB, and are validated in `requireAPIKey` middleware on the Broker. Validation looks keys up by their stored 10-character prefix, so it runs one bcrypt compare, not one per key. A 60-second in-memory cache avoids a database hit on every request. Failed attempts are rate-limited per client IP: 10 failures within 60 seconds triggers a `429`. Behind Caddy, the client IP is read from `X-Forwarded-For`, believed only from the peers listed in `TRUSTED_PROXIES`.
